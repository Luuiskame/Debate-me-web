...
const FollowersCard = ({ userId }) => {
  ...
  const unreadFollowers = useSelector(
    (state) => state.userReducer.unreadFollowers
  );
    ...
    <>
      <button
        className={`${styles.mainButtons} ${
          displayCardClass ? styles.remove : null
        }`}
      ...
      <div
        className={`${
          displayCardClass ? styles.showFollwersCard : styles.remove
        }`}
      >
